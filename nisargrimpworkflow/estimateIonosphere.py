...
def resample_vrt_to_runw(offset_m, vrt_gt, runw):
    """Bilinearly resample VRT offset data to the RUNW ML grid.
...
def _conv_fill(arr, valid, iterations):
    filled = arr.copy()
    for _ in range(iterations):
        smoothed = ndimage.convolve(filled, _FILL_KERNEL, mode='reflect')
        filled = np.where(valid, arr, smoothed)
    return filled


def _pyramid_fill(arr, valid, levels=4, iterations_per_level=50):
...
def fill_and_smooth_iono(raw_iono, valid, sigma):
    ...
    filled = _pyramid_fill(f32, valid)
...
def write_geotiff(path, data, gt, nodata=float('nan')):
...
def parse_args():
    ...
    p.add_argument('--sigma-az', type=float, default=100.0, metavar='PX',
                   help='Azimuth Gaussian sigma for iono smoothing (default: 100 px)')
...
def main():
    ...
    if args.phaseThresh is not None:
        thresh_mask = np.abs(phase_out - vel_sim) >= args.phaseThresh
        n_masked = int(np.sum(thresh_mask & np.isfinite(phase_out)))
        print(f"\nPhase threshold mask (|correctedPhase - simPhase| >= "
              f"{args.phaseThresh:.2f} rad): {n_masked:,} pixels masked.")
    ...
    write_geotiff(iono_tif, iono, gt)
    write_geotiff(phase_tif, phase_out, gt)
    if not args.noInterp:
        print("\nHole-filling correctedUnwrappedPhase with intfloat ...")
        run_intfloat(phase_tif, phase_vrt,
        ...
        _ds = gdal.Open(phase_tif, gdal.GA_Update)
        if _ds is not None:
            ...
            _ds.FlushCache()
            _ds = None
        # intfloat writes no band description; stamp it now so downstream
    ...
    if args.outputAll:
        write_geotiff(iono_unfilled_tif, iono_unfilled, gt)
        write_geotiff(offset_phase_tif, offset_phase.astype(np.float32), gt)
        write_geotiff(unwrapped_tif, phase.astype(np.float32), gt)
